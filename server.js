...
app.post("/api/orders/create", limiter, async (req, res) => {
  ...
  const order = { id: uuidv4(), tableId: table, items, status: "pending", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  if (mongoDb) {
...
app.patch("/api/orders/:id", auth, async (req, res) => {
  try {
    ...
    if (!id) return res.status(400).json({ ok: false, msg: "order id required" });
    if (!allowed.has(String(status))) return res.status(400).json({ ok: false, msg: "invalid status" });

    let updated = null;
    if (mongoDb) {
      const resUpdate = await mongoDb.collection("orders").updateOne(
        { _id: id },
        { $set: { status, updatedAt: new Date() } }
      );
      if (!resUpdate.matchedCount) return res.status(404).json({ ok: false, msg: "order not found" });
      updated = await mongoDb.collection("orders").findOne({ _id: id });
      const all = await mongoDb.collection("orders").find({}).toArray();
      io.emit("orders:update", all);
    } else {
      const d = await readData();
      const idx = (d.orders || []).findIndex(o => String(o.id) === id);
      if (idx < 0) return res.status(404).json({ ok: false, msg: "order not found" });
    ...
    res.status(500).json({ ok: false, msg: "Failed to update order" });
  }
});
...
app.get("/api/categories", auth, async (req, res) => {
  try {
    let categories = [];
    if (mongoDb) {
      categories = await mongoDb.collection("categories").find({}).sort({ displayOrder: 1 }).toArray();
    } else {
      const d = await readData();
    ...
    res.status(500).json({ ok: false, msg: "Failed to load categories" });
  }
});
...
app.post("/api/categories", auth, async (req, res) => {
  try {
    ...
    if (!nm) return res.status(400).json({ ok: false, msg: "name required" });
    const nowIso = new Date().toISOString();
    const cat = {
      id: uuidv4(),
      name: nm,
    ...
    res.status(500).json({ ok: false, msg: "Failed to add category" });
  }
});
...
app.put("/api/categories/:id", auth, async (req, res) => {
  try {
    ...
    if (mongoDb) {
      ...
      if (displayOrder !== undefined) updates.displayOrder = Number(displayOrder) || 1;
      if (typeof active === "boolean") updates.active = active;
      const u = await mongoDb.collection("categories").updateOne({ _id: id }, { $set: updates });
      ...
      const cur = d.categories[idx];
      const record = { ...cur };
      if (name !== undefined) record.name = name;
      if (displayOrder !== undefined) record.displayOrder = Number(displayOrder) || 1;
      if (typeof active === "boolean") record.active = active;
      record.updatedAt = new Date().toISOString();
      d.categories[idx] = record;
      await writeData(d);
      updated = record;
    }
    ...
    res.status(500).json({ ok: false, msg: "Failed to update category" });
  }
});

app.patch("/api/categories/:id/toggle", auth, async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    if (!id) return res.status(400).json({ ok: false, msg: "id required" });
    let updated = null;
    if (mongoDb) {
      const cur = await mongoDb.collection("categories").findOne({ _id: id });
      if (!cur) return res.status(404).json({ ok: false, msg: "not found" });
      await mongoDb.collection("categories").updateOne({ _id: id }, { $set: { active: !cur.active, updatedAt: new Date() } });
      updated = await mongoDb.collection("categories").findOne({ _id: id });
    } else {
      const d = await readData();
      d.categories = d.categories || [];
      const idx = d.categories.findIndex(c => String(c.id) === id);
      if (idx < 0) return res.status(404).json({ ok: false, msg: "not found" });
      d.categories[idx].active = !d.categories[idx].active;
      d.categories[idx].updatedAt = new Date().toISOString();
      await writeData(d);
    ...
    res.status(500).json({ ok: false, msg: "Failed to toggle category" });
  }
});
...
app.delete("/api/categories/:id", auth, async (req, res) => {
  try {
    ...
    if (!id) return res.status(400).json({ ok: false, msg: "id required" });
    if (mongoDb) {
      const r = await mongoDb.collection("categories").deleteOne({ _id: id });
      if (!r.deletedCount) return res.status(404).json({ ok: false, msg: "not found" });
      await mongoDb.collection("menu").deleteMany({ categoryId: id });
    } else {
      const d = await readData();
      d.categories = (d.categories || []).filter(c => String(c.id) !== id);
      d.menu = (d.menu || []).filter(m => String(m.categoryId) !== id);
      await writeData(d);
    }
    res.json({ ok: true });
  } catch (err) {
    console.error("Delete category failed", err);
    res.status(500).json({ ok: false, msg: "Failed to delete category" });
  }
});

// Categories API (admin-protected)
app.get("/api/categories", auth, async (req, res) => {
  try {
    let categories = [];
    if (mongoDb) {
...
app.get("/api/menu", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, msg: "Failed to load menu" });
  }
});

app.post("/api/menu", auth, async (req, res) => {
  try {
    const { name, description, price, categoryId, available, imageUrl, prepTime } = req.body || {};
    if (!name || !categoryId) return res.status(400).json({ ok: false, msg: "name and categoryId required" });
    const item = { id: uuidv4(), name, description: description||"", price: Number(price)||0, categoryId: String(categoryId), available: available!==false, imageUrl: imageUrl||"", prepTime: Number(prepTime)||0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    if (mongoDb) {
      await mongoDb.collection("menu").insertOne({ ...item, _id: item.id });
    } else {
      const d = await readData();
      d.menu = d.menu || [];
      d.menu.push(item);
    ...
    res.status(500).json({ ok: false, msg: "Failed to add menu item" });
  }
});
...
app.put("/api/menu/:id", auth, async (req, res) => {
  try {
    ...
    let updated = null;
    if (mongoDb) {
      const u = await mongoDb.collection("menu").updateOne({ _id: id }, { $set: { name, description, price: Number(price)||0, categoryId: String(categoryId), available: !!available, imageUrl: imageUrl||"", prepTime: Number(prepTime)||0, updatedAt: new Date() } });
      if (!u.matchedCount) return res.status(404).json({ ok: false, msg: "not found" });
      updated = await mongoDb.collection("menu").findOne({ _id: id });
    } else {
      const d = await readData();
      d.menu = d.menu || [];
      const idx = d.menu.findIndex(m => String(m.id) === id);
      if (idx < 0) return res.status(404).json({ ok: false, msg: "not found" });
      d.menu[idx] = { ...d.menu[idx], name, description, price: Number(price)||0, categoryId: String(categoryId), available: !!available, imageUrl: imageUrl||"", prepTime: Number(prepTime)||0, updatedAt: new Date().toISOString() };
      await writeData(d);
      updated = d.menu[idx];
    }
    ...
    res.status(500).json({ ok: false, msg: "Failed to update menu item" });
  }
});

app.patch("/api/menu/:id/toggle", auth, async (req, res) => {
  try {
    const id = String(req.params.id||"").trim();
    if (!id) return res.status(400).json({ ok: false, msg: "id required" });
    let updated = null;
    if (mongoDb) {
      const cur = await mongoDb.collection("menu").findOne({ _id: id });
      if (!cur) return res.status(404).json({ ok: false, msg: "not found" });
      await mongoDb.collection("menu").updateOne({ _id: id }, { $set: { available: !cur.available, updatedAt: new Date() } });
      updated = await mongoDb.collection("menu").findOne({ _id: id });
    } else {
      const d = await readData();
      d.menu = d.menu || [];
      const idx = d.menu.findIndex(m => String(m.id) === id);
      if (idx < 0) return res.status(404).json({ ok: false, msg: "not found" });
      d.menu[idx].available = !d.menu[idx].available;
      d.menu[idx].updatedAt = new Date().toISOString();
      await writeData(d);
      updated = d.menu[idx];
    }
    res.json({ ok: true, item: updated });
  } catch (err) {
    console.error("Toggle menu item failed", err);
    res.status(500).json({ ok: false, msg: "Failed to toggle menu item" });
  }
});
...
app.delete("/api/menu/:id", auth, async (req, res) => {
  try {
    ...
    if (!id) return res.status(400).json({ ok: false, msg: "id required" });
    if (mongoDb) {
      const r = await mongoDb.collection("menu").deleteOne({ _id: id });
      if (!r.deletedCount) return res.status(404).json({ ok: false, msg: "not found" });
    } else {
      const d = await readData();
      d.menu = (d.menu || []).filter(m => String(m.id) !== id);
      await writeData(d);
    }
    res.json({ ok: true });
  } catch (err) {
    console.error("Delete menu item failed", err);
    res.status(500).json({ ok: false, msg: "Failed to delete menu item" });
  }
});
...
app.get("/api/public/menu", async (req, res) => {
  try {
    ...
    const activeCatIds = new Set(categories.filter(c => c.active !== false).map(c => String(c.id)));
    let items = menu.filter(m => activeCatIds.has(String(m.categoryId)) && m.available !== false);
    if (categoryId) items = items.filter(m => String(m.categoryId) === String(categoryId));
    if (categoryName) {
      const match = categories.find(c => (c.name||"").toLowerCase() === String(categoryName).toLowerCase());
      if (match) items = items.filter(m => String(m.categoryId) === String(match.id));
      else items = [];
    }
    res.json({ ok: true, menu: items });
  } catch (err) {
...
// Public categories for index page
